...
from training.base_strategy import TrainingStrategy
        ...
        if self.lr_scheduler_type == "linear-warmup+cosine-decay":
            ...
            for name, param in self.vlm.named_parameters():
                ...
                if "multi_modal_projector" in name:
                    mm_proj.append(param)
                elif "video_projecter" in name:
                    video_proj.append(param)
                elif "lm_head" in name:
                    lm_head.append(param)
                elif "embed_tokens" in name:
                    embed_tokens.append(param)
                elif "lora" in name:
                    lora.append(param)
            ...
            if self.args.stage == 'pretrain':
                groups = [
                    {"params": mm_proj, "weight_decay": 0.0, "lr": self.args.mm_proj_lr},
                    {"params": video_proj, "weight_decay": 0.0, "lr": self.args.lr}, 
                    ]
            elif self.args.stage == 'grounded':
                groups = [
